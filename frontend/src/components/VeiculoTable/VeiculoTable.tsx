  ...
  return (
    <tr className="animate-pulse">
      ...
      <td className="vt-cell">
        <div className="h-12 w-12 rounded-lg bg-gray-100 dark:bg-gray-700/50" />
      </td>
      {/* Text columns */}
      {Array.from({ length: COLUMN_COUNT - 1 }).map((_, i) => (
        <td key={i} className="vt-cell">
          <div className="h-4 w-16 rounded-md bg-gray-100 dark:bg-gray-700/50" />
        </td>
      ))}
    </tr>
  );
...
const thBase =
  "vt-header-cell text-left text-xs font-semibold tracking-wider text-gray-500 uppercase dark:text-gray-400";
const thRight =
  "vt-header-cell text-right text-xs font-semibold tracking-wider text-gray-500 uppercase dark:text-gray-400";

function PhotoPlaceholder() {
  return (
    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-gray-100 text-gray-400 dark:bg-gray-800 dark:text-gray-500">
...
export function VeiculoTable({
  ...
  const headers = [
    ...
    { label: "Placa", align: "left" },
    { label: "Marca", align: "left" },
    { label: "Modelo", align: "left" },
    { label: "Ano", align: "left" },
    { label: "Assessoria", align: "left" },
    { label: "Status", align: "left" },
    { label: "Data Registro", align: "left" },
    { label: "Data Apreensão", align: "left" },
    { label: "Cidade", align: "left" },
    { label: "Valor Serviço", align: "right" },
    { label: "Custo Operação", align: "right" },
    { label: "Valor Recebido", align: "right" },
    { label: "Ano (Reg)", align: "left" },
    { label: "Semana ISO (Reg)", align: "left" },
    { label: "Quem Executou", align: "left" },
      ...
      <div className="vt-scroll-container">
        <table className="vt-table">
          <thead className="vt-thead">
            <tr>
              {headers.map((h) => (
                <th key={h.label} className={h.align === "right" ? thRight : thBase}>
                  {h.label}
                </th>
              ))}
            </tr>
        ...
        <table className="vt-table">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700/30">
            {veiculos.map((v) => {
              ...
              const hasLongObs = (v.observacoes?.length || 0) > 40;

              return (
                <tr
                  key={v.id}
                  className="group transition-colors hover:bg-primary-50/40 dark:hover:bg-primary-900/10"
                >
                  {/* Foto */}
                  <td className="vt-cell">
                    {primeiraFoto ? (
                      <img
                        ...
                        className="h-12 w-12 shrink-0 rounded-lg border border-gray-200 bg-gray-50 object-cover dark:border-gray-600 dark:bg-gray-800"
                      />
                    ) : (
                      <PhotoPlaceholder />
                    )}
                  ...
                  {/* Placa */}
                  <td className="vt-cell whitespace-nowrap font-mono text-sm font-semibold text-gray-900 uppercase dark:text-gray-100">
                    {v.placa}
                  </td>

                  {/* Marca */}
                  <td className="vt-cell whitespace-nowrap text-gray-700 dark:text-gray-300">
                    {v.marca || "\u2014"}
                  </td>
                  ...
                  {/* Ano */}
                  <td className="vt-cell whitespace-nowrap tabular-nums text-gray-600 dark:text-gray-400">
                    {v.ano || "\u2014"}
                  </td>
                  ...
                  {/* Status */}
                  <td className="vt-cell whitespace-nowrap">
                    <span
                  ...
                  {/* Data Registro */}
                  <td className="vt-cell whitespace-nowrap tabular-nums text-gray-500 dark:text-gray-400">
                    {formatDate(v.data)}
                  </td>
                  ...
                  {/* Valor Serviço */}
                  <td className="vt-cell vt-currency whitespace-nowrap">
                  ...
                  {/* Valor Recebido */}
                  <td className="vt-cell vt-currency vt-currency-positive whitespace-nowrap">
                  ...
                  {/* Quem Executou */}
                  <td className="vt-cell max-w-[140px] truncate text-gray-700 dark:text-gray-300" title={v.quem_executou || ""}>
                    {v.quem_executou || "\u2014"}
                  </td>

                  {/* Observações */}
                  <td className="vt-cell max-w-[200px]">
                    {v.observacoes ? (
                      <div className="relative">
                        <span
                          className={`block text-gray-600 dark:text-gray-400 ${hasLongObs && expandedObs !== v.id ? "truncate" : ""}`}
                          title={v.observacoes}
                        >
                          {v.observacoes}
                        </span>
                        {hasLongObs && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              setExpandedObs(expandedObs === v.id ? null : v.id);
                            }}
                            className="mt-0.5 text-xs text-primary-500 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300"
                          >
                            {expandedObs === v.id ? "menos" : "mais..."}
                          </button>
                        )}
                      </div>
                    ) : (
                  ...
                  {/* Ações */}
                  <td className="vt-cell whitespace-nowrap text-right">
